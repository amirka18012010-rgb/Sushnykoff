...
async function isBlocked(userId) {
  const { data, error } = await supabase.from('users').select('is_blocked').eq('id', userId).single();
...
async function getCartFromDB(userId) {
  ...
  if (error) return [];
  return data.map(row => ({ productId: row.product_id, quantity: row.quantity, priceType: row.price_type, price: row.price }));
}
async function setCartToDB(userId, cart) {
  await supabase.from('cart_items').delete().eq('user_id', userId);
  if (cart.length === 0) return;
  const items = cart.map(item => ({ user_id: userId, product_id: item.productId, quantity: item.quantity, price_type: item.priceType, price: item.price }));
  await supabase.from('cart_items').insert(items);
}

// ============================================================
// ПУБЛИЧНЫЕ МАРШРУТЫ (СОКРАЩЕННЫЕ ДЛЯ ЯСНОСТИ)
// ============================================================
...
app.get('/api/news/latest', async (req, res) => { try { const { data, error } = await supabase.from('news').select('*').eq('is_active', true).order('created_at', { ascending: false }).limit(3); if (error) throw error; res.json(data); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/settings', async (req, res) => { try { const { data, error } = await supabase.from('settings').select('key, value'); if (error) throw error; const settings = {}; data.forEach(row => settings[row.key] = row.value); res.json(settings); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/notifications', async (req, res) => { try { const { data, error } = await supabase.from('notifications').select('*').eq('is_read', 0).order('created_at', { ascending: false }); if (error) throw error; res.json(data); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/notifications/read', async (req, res) => { try { await supabase.from('notifications').update({ is_read: 1 }).eq('is_read', 0); res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
...
// АДМИН-МАРШРУТЫ
// ============================================================
app.put('/api/admin/settings', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const updates = req.body; for (const [key, value] of Object.entries(updates)) await supabase.from('settings').update({ value }).eq('key', key); res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/admin/login', (req, res) => { try { const { login, password } = req.body; const adminLogin = process.env.ADMIN_LOGIN || 'admin'; const adminPassword = process.env.ADMIN_PASSWORD || 'admin123'; if (login === adminLogin && password === adminPassword) { req.session.isAdmin = true; res.json({ success: true }); } else res.status(401).json({ error: 'Неверный логин или пароль' }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/admin/logout', (req, res) => { req.session.destroy(); res.json({ success: true }); });
app.get('/api/admin/status', (req, res) => { res.json({ isAdmin: !!req.session.isAdmin }); });

// ---- Админ: товары ----
app.get('/api/admin/products', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { data, error } = await supabase.from('products').select(`*, categories:category_id(name), brands:brand_id(name), volumes:volume_id(name)`).order('id', { ascending: false }).limit(250); if (error) throw error; const items = data.map(p => ({ ...p, category_name: p.categories?.name, brand_name: p.brands?.name, volume_name: p.volumes?.name })); res.json(items); } catch (err) { res.status(500).json({ error: err.message }); } });

// ---- Админ: добавление товара (Здесь исправлена ошибка с Supabase .catch) ----
app.post('/api/admin/products', upload.single('image'), async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('products').insert({
      name, price: parseFloat(price), category_id, brand_id, volume_id, image,
      description: description || '', wholesale_price: wholesale_price ? parseFloat(wholesale_price) : null
    }).select('id');
    if (error) { console.error('❌ ОШИБКА ПРИ ВСТАВКЕ ТОВАРА В БАЗУ:', error); throw error; }
    
    // ПРАВИЛЬНЫЙ СПОСОБ: .then({error}) вместо .catch()
    supabase.from('notifications').insert({ message: 'Добавлен новый товар: ' + name })
      .then(({ error: notifError }) => {
        if (notifError) console.error('Ошибка отправки уведомления:', notifError);
      });
...
app.put('/api/admin/products/:id', upload.single('image'), async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const id = req.params.id; const { name, price, category_id, brand_id, volume_id, imageUrl, description, wholesale_price } = req.body; let image = ''; if (req.file) { const publicUrl = await uploadToStorage(req.file, 'products'); if (publicUrl) image = publicUrl; } else if (imageUrl && imageUrl.trim() !== '') { image = imageUrl.trim(); } else { const { data: old } = await supabase.from('products').select('image').eq('id', id).single(); if (old) image = old.image; } const { error } = await supabase.from('products').update({ name, price: parseFloat(price), category_id, brand_id, volume_id, image, description: description || '', wholesale_price: wholesale_price ? parseFloat(wholesale_price) : null }).eq('id', id); if (error) throw error; res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.delete('/api/admin/products/:id', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { error } = await supabase.from('products').delete().eq('id', req.params.id); if (error) throw error; res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });

// ---- Админ: категории, бренды, объёмы, заказы, пользователи, новости, фон ----
app.get('/api/admin/categories', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { data, error } = await supabase.from('categories').select('*').order('name'); if (error) throw error; res.json(data); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/admin/categories', upload.single('categoryImage'), async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { name, icon, imageUrl } = req.body; let image = ''; if (req.file) { const publicUrl = await uploadToStorage(req.file, 'categories'); if (publicUrl) image = publicUrl; } else if (imageUrl && imageUrl.trim() !== '') { image = imageUrl.trim(); } if (!name) return res.status(400).json({ error: 'Введите название категории' }); const { data, error } = await supabase.from('categories').insert({ name, icon: icon || '', image }).select('id'); if (error) throw error; res.json({ id: data[0].id }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.put('/api/admin/categories/:id', upload.single('categoryImage'), async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const id = req.params.id; const { name, icon, imageUrl } = req.body; let image = ''; if (req.file) { const publicUrl = await uploadToStorage(req.file, 'categories'); if (publicUrl) image = publicUrl; } else if (imageUrl && imageUrl.trim() !== '') { image = imageUrl.trim(); } else { const { data: old } = await supabase.from('categories').select('image').eq('id', id).single(); if (old) image = old.image; } const { error } = await supabase.from('categories').update({ name, icon: icon || '', image }).eq('id', id); if (error) throw error; res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.delete('/api/admin/categories/:id', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { error } = await supabase.from('categories').delete().eq('id', req.params.id); if (error) throw error; res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/admin/brands', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { data, error } = await supabase.from('brands').select(`*, categories:category_id(name)`).order('name'); if (error) throw error; const items = data.map(b => ({ ...b, category_name: b.categories?.name })); res.json(items); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/admin/brands', upload.single('image'), async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { name, description, category_id, imageUrl } = req.body; let image = ''; if (req.file) { const publicUrl = await uploadToStorage(req.file, 'brands'); if (publicUrl) image = publicUrl; } else if (imageUrl && imageUrl.trim() !== '') { image = imageUrl.trim(); } if (!name || !category_id) return res.status(400).json({ error: 'Заполните название и категорию' }); const { data, error } = await supabase.from('brands').insert({ name, description: description || '', image, category_id }).select('id'); if (error) throw error; res.json({ id: data[0].id }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.put('/api/admin/brands/:id', upload.single('image'), async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const id = req.params.id; const { name, description, category_id, imageUrl } = req.body; let image = ''; if (req.file) { const publicUrl = await uploadToStorage(req.file, 'brands'); if (publicUrl) image = publicUrl; } else if (imageUrl && imageUrl.trim() !== '') { image = imageUrl.trim(); } else { const { data: old } = await supabase.from('brands').select('image').eq('id', id).single(); if (old) image = old.image; } const { error } = await supabase.from('brands').update({ name, description: description || '', image, category_id }).eq('id', id); if (error) throw error; res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.delete('/api/admin/brands/:id', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { error } = await supabase.from('brands').delete().eq('id', req.params.id); if (error) throw error; res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/admin/volumes', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { data, error } = await supabase.from('volumes').select('*').order('sort_order', { ascending: true }); if (error) throw error; res.json(data); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/admin/volumes', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { name, sort_order } = req.body; if (!name) return res.status(400).json({ error: 'Введите название объёма' }); const { data, error } = await supabase.from('volumes').insert({ name, sort_order: sort_order || 0 }).select('id'); if (error) throw error; res.json({ id: data[0].id }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.put('/api/admin/volumes/:id', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { name, sort_order } = req.body; if (!name) return res.status(400).json({ error: 'Введите название объёма' }); const { error } = await supabase.from('volumes').update({ name, sort_order: sort_order || 0 }).eq('id', req.params.id); if (error) throw error; res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.delete('/api/admin/volumes/:id', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { error } = await supabase.from('volumes').delete().eq('id', req.params.id); if (error) throw error; res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/admin/orders', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { data, error } = await supabase.from('orders').select(`*, users:user_id(first_name, last_name, login)`).order('created_at', { ascending: false }).limit(250); if (error) throw error; const items = data.map(o => ({ ...o, first_name: o.users?.first_name, last_name: o.users?.last_name, login: o.users?.login })); res.json(items); } catch (err) { res.status(500).json({ error: err.message }); } });
app.put('/api/admin/orders/:id', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { status } = req.body; if (!status) return res.status(400).json({ error: 'Укажите статус' }); const { data: order } = await supabase.from('orders').select('user_id').eq('id', req.params.id).single(); if (!order) return res.status(404).json({ error: 'Заказ не найден' }); const { error } = await supabase.from('orders').update({ status }).eq('id', req.params.id); if (error) throw error; const statusMap = { pending: 'ожидает подтверждения', paid: 'оплачен', shipped: 'отправлен', delivered: 'доставлен' }; const statusText = statusMap[status] || status; const message = `📦 Статус заказа №${req.params.id} изменён на «${statusText}»`; await supabase.from('notifications').insert({ message }); res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/admin/users', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { data, error } = await supabase.from('users').select('id, first_name, last_name, login, is_blocked, created_at, phone').order('id', { ascending: false }).limit(250); if (error) throw error; res.json(data); } catch (err) { res.status(500).json({ error: err.message }); } });
app.put('/api/admin/users/:id/block', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { block } = req.body; const { error } = await supabase.from('users').update({ is_blocked: block ? 1 : 0 }).eq('id', req.params.id); if (error) throw error; res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.delete('/api/admin/users/:id', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { error } = await supabase.from('users').delete().eq('id', req.params.id); if (error) throw error; res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/admin/news', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { data, error } = await supabase.from('news').select('*').order('created_at', { ascending: false }).limit(250); if (error) throw error; res.json(data); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/admin/news', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { title, content, is_active } = req.body; if (!title || !content) return res.status(400).json({ error: 'Заполните заголовок и текст' }); const { data, error } = await supabase.from('news').insert({ title, content, is_active: is_active || 1 }).select('id'); if (error) throw error; await supabase.from('notifications').insert({ message: '📢 Новая новость: ' + title }); res.json({ id: data[0].id }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.put('/api/admin/news/:id', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { title, content, is_active } = req.body; const { error } = await supabase.from('news').update({ title, content, is_active }).eq('id', req.params.id); if (error) throw error; res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.delete('/api/admin/news/:id', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { error } = await supabase.from('news').delete().eq('id', req.params.id); if (error) throw error; res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/admin/upload-background', upload.single('background'), async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); if (!req.file) return res.status(400).json({ error: 'Файл не загружен' }); const publicUrl = await uploadToStorage(req.file, 'backgrounds'); if (!publicUrl) return res.status(500).json({ error: 'Не удалось загрузить изображение' }); await supabase.from('settings').update({ value: publicUrl }).eq('key', 'site_background'); res.json({ success: true, path: publicUrl }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.put('/api/admin/background', async (req, res) => { try { if (!isAdmin(req)) return res.status(403).json({ error: 'Доступ запрещён' }); const { url } = req.body; await supabase.from('settings').update({ value: url || '' }).eq('key', 'site_background'); res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
