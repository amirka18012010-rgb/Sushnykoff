...
// ---- Функция сжатия через Sharp (ТЕПЕРЬ ПОТОКОВАЯ, БЕЗ ЗАГРУЗКИ В ПАМЯТЬ) ----
async function uploadToStorage(file, folder) {
  if (!file) return null;
  try {
    // Передаём путь к файлу напрямую в sharp. Он сам прочитает его потоком!
    const optimizedBuffer = await sharp(file.path)
      .resize(800, null, { withoutEnlargement: true })
      .webp({ quality: 80 })
      .toBuffer();

    // Удаляем временный файл после обработки (обязательно!)
