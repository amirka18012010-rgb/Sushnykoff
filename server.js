...
// В самом начале server.js, после require
const fs = require('fs');
...
db.serialize(() => {
  ...
  const defaultSettings = {
    about_text: 'Мы — команда энтузиастов, которые любят вкусные напитки. Наш магазин предлагает широкий ассортимент газировок, соков, воды и энергетиков. Мы работаем с 2025 года и постоянно расширяем ассортимент. Наша миссия — сделать покупку напитков удобной и приятной.',
  ...
  db.get('SELECT * FROM admin WHERE login = ?', [adminLogin], (err, row) => {
    if (!row) db.run('INSERT INTO admin (login, password) VALUES (?, ?)', [adminLogin, adminPass]);
  });
});
...
// 2. Товары (фильтр, поиск, пагинация)
...
app.post('/api/products/:id/reviews', (req, res) => {
  ...
  db.run(
    ...
    function(err) {
      if (err) return res.status(500).json({ error: err.message });
      db.get('SELECT AVG(rating) as avg FROM reviews WHERE product_id = ?', [req.params.id], (err, row) => {
        if (!err && row) {
          db.run('UPDATE products SET avg_rating = ? WHERE id = ?', [row.avg || 0, req.params.id]);
        }
      });
...
app.get('/api/orders/history', (req, res) => {
  ...
  db.all('SELECT * FROM orders WHERE user_id = ? AND is_deleted = 0 ORDER BY created_at DESC', [userId], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(rows);
  });
});
...
app.delete('/api/orders/history', (req, res) => {
  if (!isAuthenticated(req)) return res.status(401).json({ error: 'Необходимо войти' });
  const userId = req.session.userId;
  db.run('UPDATE orders SET is_deleted = 1 WHERE user_id = ?', [userId], function(err) {
    if (err) return res.status(500).json({ error: err.message });
...
// ============================================================
// ГЛАВНЫЙ МАРШРУТ ВХОДА АДМИНА (исправлен, работает)
// ============================================================
...
// ----- Админ: заказы (все, включая удалённые пользователем) -----
